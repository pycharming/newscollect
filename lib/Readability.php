...
class Readability {
    ...
    private $junkTagsById = Array("agreementtxt");
    // 需要删除的标签
     ...
     * @return DOMDocument
     */
    private function removeJunkTagById($RootNode, $id) {
        $Tag = $RootNode->getElementById($id);
if($Tag) {
...
$parentNode->removeChild($Tag);
    ...
    private function getTopBox() {
        ...
        while($paragraph = $allParagraphs->item($i++)) {
            ...
            if (preg_match("/(comment|meta|footer|footnote|agreement|hot_|news_bl|info_|keyword)/i", $className)) {
                $contentScore -= 50;
            } else if(preg_match(
                "/((^|\\s)(news_txt|post|hentry|article|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)(\\s|$))/i",
                $className)) {
                $contentScore += 25;
            }

            // Look for a special ID
            if (preg_match("/(comment|meta|footer|footnote|agreement)/i", $id)) {
                $contentScore -= 50;
            } else if (preg_match(
    ...
    public function getContent() {
        ...
        // 删除不需要的标签（通过 id）
        foreach ($this->junkTagsById as $id) {
            $this->removeJunkTagById($this->DOM, $id);
        }
        // 删除不需要的标签
        foreach ($this->junkTags as $tag) {
            $this->removeJunkTag($this->DOM, $tag);
        }

        // 删除不需要的属性
        foreach ($this->junkAttrs as $attr) {
            $this->removeJunkAttr($this->DOM, $attr);
        }
        ...
        if($ContentBox === null)
            return false; //exit('no found content');
